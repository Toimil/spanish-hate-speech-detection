...
#############################################################
#############################################################
# COPIA DE LA FUNCION preprocess_tweet DE PYSENTIMIENTO
#############################################################
#############################################################
...
def preprocess_tweet(
    ...
    def process_hashtags(x):
        ...
        if hashtag_token:
            text = hashtag_token + " " + text + " " + hashtag_token
...
def lower_stopwords(text):
    return " ".join([word for word in text.lower().split() if word not in stop_words])

def lower_preprocess_without_url(text):
    return preprocess_tweet(
        text, lang="es", user_token=None, url_token=' ', preprocess_hashtags=True, hashtag_token=None, char_replace=True,
        demoji=True, shorten=3, normalize_laughter=True, emoji_wrapper="<emoji>", preprocess_handles=True).lower()

def lower_preprocess_without_url_stopwords(text):
    return " ".join([word for word in preprocess_tweet(
        text, lang="es", user_token=None, url_token=' ', preprocess_hashtags=True, hashtag_token=None, char_replace=True,
        demoji=True, shorten=3, normalize_laughter=True, emoji_wrapper="<emoji>", preprocess_handles=True).lower().split() if word not in stop_words])

def lower_preprocess_without_emojis_and_url(text):
    return delete_emojis(preprocess_tweet(
        text, lang="es", user_token=None, url_token=' ', preprocess_hashtags=True, hashtag_token=None, char_replace=True,
        demoji=False, shorten=3, normalize_laughter=True, emoji_wrapper="<emoji>", preprocess_handles=True).lower())

def lower_preprocess_without_emojis_and_url_stopwords(text):
    return " ".join([word for word in delete_emojis(preprocess_tweet(
        text, lang="es", user_token=None, url_token=' ', preprocess_hashtags=True, hashtag_token=None, char_replace=True,
        demoji=False, shorten=3, normalize_laughter=True, emoji_wrapper="<emoji>", preprocess_handles=True).lower()).split() if word not in stop_words])

def lower_preprocess_without_hashtags_and_emojis_and_url(text):
    return delete_hashtags(delete_emojis(preprocess_tweet(
        text, lang="es", user_token=None, url_token=' ', preprocess_hashtags=False, hashtag_token=' ', char_replace=True,
        demoji=False, shorten=3, normalize_laughter=True, emoji_wrapper="<emoji>", preprocess_handles=True).lower()))

def lower_preprocess_without_hashtags_and_emojis_and_url_stopwords(text):
    return " ".join([word for word in delete_hashtags(delete_emojis(preprocess_tweet(
        text, lang="es", user_token=None, url_token=' ', preprocess_hashtags=False, hashtag_token=' ', char_replace=True,
        demoji=False, shorten=3, normalize_laughter=True, emoji_wrapper="<emoji>", preprocess_handles=True))).lower().split() if word not in stop_words])

def lower_preprocess_without_user_and_hashtags_and_emojis_and_url(text):
    return delete_hashtags(delete_emojis(preprocess_tweet(
        text, lang="es", user_token=' ', url_token=' ', preprocess_hashtags=False, hashtag_token=' ', char_replace=True,
        demoji=False, shorten=3, normalize_laughter=True, emoji_wrapper=" ", preprocess_handles=True))).lower()

def lower_preprocess_without_user_and_hashtags_and_emojis_and_url_stopwords(text):
    return " ".join([word for word in delete_hashtags(delete_emojis(preprocess_tweet(
        text, lang="es", user_token=' ', url_token=' ', preprocess_hashtags=False, hashtag_token=' ', char_replace=True,
        demoji=False, shorten=3, normalize_laughter=True, emoji_wrapper=" ", preprocess_handles=True))).lower().split() if word not in stop_words])
...
preprocess_variants = {
    "lower": lower,
    "lower_stopwords": lower_stopwords,
    "lower_preprocess_without_url": lower_preprocess_without_url,
    "lower_preprocess_without_url_stopwords": lower_preprocess_without_url_stopwords,
    "lower_preprocess_without_emojis_and_url": lower_preprocess_without_emojis_and_url,
    "lower_preprocess_without_emojis_and_url_stopwords": lower_preprocess_without_emojis_and_url_stopwords,
    "lower_preprocess_without_hashtags_and_emojis_and_url": lower_preprocess_without_hashtags_and_emojis_and_url,
    "lower_preprocess_without_hashtags_and_emojis_and_url_stopwords": lower_preprocess_without_hashtags_and_emojis_and_url_stopwords,
    "lower_preprocess_without_user_and_hashtags_and_emojis_and_url": lower_preprocess_without_user_and_hashtags_and_emojis_and_url,
    "lower_preprocess_without_user_and_hashtags_and_emojis_and_url_stopwords": lower_preprocess_without_user_and_hashtags_and_emojis_and_url_stopwords,
}
...
# GUARDAR VARIANTES
def guardar_variantes(df, base_output_name, es_test=False):
    for nombre, funcion in preprocess_variants.items():
        df_copy = df.copy()
        df_copy["text"] = df_copy["text"].apply(funcion)
        tipo = "test" if es_test else "train"
        output_name = f"{base_output_name}_{tipo}_{nombre}.csv"
        df_copy.to_csv(output_name, index=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_prefix", default="ibereval", help="Prefijo para nombres de archivos de salida")
    args = parser.parse_args()

    df_train = cargar_dataset(args.train_file)
    df_test = cargar_dataset(args.test_file)

    guardar_variantes(df_train, args.output_prefix, es_test=False)
    guardar_variantes(df_test, args.output_prefix, es_test=True)
