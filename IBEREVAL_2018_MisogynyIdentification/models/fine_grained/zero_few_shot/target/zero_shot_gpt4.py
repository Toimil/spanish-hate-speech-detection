...
def construir_prompt(texto):
    ...
    - 'active' : activo (individual), el texto incluye mensajes ofensivos dirigidos deliberadamente a un objetivo específico.
    - 'passive' : pasivo (genérico), se refiere a mensajes publicados a muchos destinatarios potenciales.
    - '0' : Si el texto no es misógino


    Decide la categoría del siguiente texto. Responde únicamente con 'active' , 'passive' o '0'.
...
def zero_shot_clasificar(textos):
    ...
    for texto in textos:
        ...
        try:
            ...
            if 'active' in resultado or 'Active' in resultado:
                pred = 'active'
            elif 'passive' in resultado or 'Passive' in resultado:
                pred = 'passive'
            else:
                pred = '0'
...
if __name__ == "__main__":
    ...
    evaluar(df_test["target"], test_preds, "test")
